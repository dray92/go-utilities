...
func GenerateIntermediateCert(
	...
	if err != nil {
		return nil, nil, errors.New("failed to make a copy of the settings template")
	}
	...
	templateCopy.MaxPathLenZero = templateCopy.MaxPathLen < 1
...
func VerifyIntermediateCert(root, intermediate *x509.Certificate) (bool, error) {
	...
	if _, err := intermediate.Verify(opts); err != nil {
		return false, errors.Wrap(err, "failed to verify intermediate cert")
	}
	return true, nil
}
...
func VerifyLeafCert(root, intermediate, child *x509.Certificate) (bool, error) {
	roots := x509.NewCertPool()
	...
	if _, err := child.Verify(opts); err != nil {
		return false, errors.Wrap(err, "failed to verify leaf cert")
	}
	return true, nil
}
...
func generateX509Cert(
	...
	return cert, pemEncodedCert, nil
}
